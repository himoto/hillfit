...
class HillFit(object):
    def __init__(self, x_data: Union[list, ndarray], y_data: Union[list, ndarray]):
        self.x_data = x_data
        self.y_data = y_data
    ...
    def regression(self, x_fit, y_fit, view_figure, x_label, y_axis, title, *params):        
    ...
    def fitting(self, x_label = 'x', y_axis = 'y', title = 'Fitted Hill equation', num = None, sigfigs = 6, view_figure = True):
        ...
        self.equation = f'Fitted Hill equation: {round(self.bottom, sigfigs)} + ({round(self.top, sigfigs)}-{round(self.bottom, sigfigs)})*x^{(round(self.nH, sigfigs))} / ({round(self.ec50, sigfigs)}^{(round(self.nH, sigfigs))} + x^{(round(self.nH, sigfigs))})'
        
        self.regression(self.x_fit, self.y_fit, view_figure, x_label, y_axis, title, *params)
    ...
    def export(self, export_path = None, export_name = None):
        ...
        string = '\n'.join([self.equation, f'top: {self.top}', f'bottom: {self.bottom}', f'ec50: {self.ec50}', f'nH: {self.nH}'])
